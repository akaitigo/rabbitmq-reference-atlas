...
def scenario_axis(reference_axis: dict, scenario: str, rows: list[dict], proof_index: dict) -> dict:
    ...
    observed = [row for row in applicable if proof_by_pair[(row["behavior_id"], scenario)]["legacy_runtime_observation"]]
    closed = [row for row in applicable if proof_by_pair[(row["behavior_id"], scenario)]["scenario_gap_closed"]]
    eligible = [row for row in applicable if proof_by_pair[(row["behavior_id"], scenario)]["completion_eligible"]]
    gaps = [row["proof_obligation_id"] for row in applicable if row not in closed]
    partitions: dict[str, dict[str, int]] = defaultdict(lambda: {
        "required": 0, "legacy_runtime_observation": 0, "scenario_gap_closed": 0, "completion_eligible": 0})
    for row in applicable:
        partitions[row["profile"]]["required"] += 1
        if row in observed:
            partitions[row["profile"]]["legacy_runtime_observation"] += 1
        if row in closed:
            partitions[row["profile"]]["scenario_gap_closed"] += 1
        if row in eligible:
            partitions[row["profile"]]["completion_eligible"] += 1
    check = make_check(
        f"{reference_axis['id']}.dedicated-proof",
        "各Required Surface×Scenarioの全Variantを専用実Broker/Clientでretry 0実行し、Oracle、Source/Harness digest、Runtime identity、packet/log/metric Artifactを接続する。",
        f"legacy-observation={len(observed)}/{len(applicable)}; scenario-gap-closed={len(closed)}/{len(applicable)}; completion-eligible={len(eligible)}/{len(applicable)}",
        "pass" if not gaps else ("partial" if closed else "gap"),
        ["scenario-closure.yaml", "verification.plan.yaml", "evidence/scenarios/index.json", "evidence/scenarios/behaviors/"], gaps)
    return make_axis(reference_axis, {
        "unit": f"authority-derived behavior requiring {scenario}",
        "required": len(applicable), "legacy_runtime_observation": len(observed),
        "scenario_gap_closed": len(closed), "completion_eligible": len(eligible),
        "remaining": len(applicable) - len(closed),
...
def build() -> dict:
    ...
    for row in covered_rows:
        ...
        if not record or record.get("source_digest") != current_lock or record.get("execution_mode") not in {"runtime", "platform"}:
            binding_gaps.append(row["proof_obligation_id"])
    closed_pairs = {(item["behavior_id"], item["scenario"]) for item in scenario_proofs["files"] if item["scenario_gap_closed"]}
    uncovered_artifacts = [row["proof_obligation_id"] for row in required_rows if (row["behavior_id"], row["scenario"]) not in closed_pairs]
    artifact_axis = make_axis(axes_by_id["artifact-trace"], {
        ...
        "legacy_runtime_observation": scenario_proofs["summary"]["legacy_runtime_observation_rows"],
        "dedicated_runtime_reports": scenario_proofs["summary"]["dedicated_runtime_report_rows"],
        "scenario_gap_closed": scenario_proofs["summary"]["scenario_gap_closed_rows"],
        ...
        make_check("artifact.unique-binding", "各Required rowの全Variantへ共有しないpacket/log/metric Artifactを専用pathとdigestで接続する。",
                   f"legacy-observation={scenario_proofs['summary']['legacy_runtime_observation_rows']}/{len(required_rows)}; scenario-gap-closed={scenario_proofs['summary']['scenario_gap_closed_rows']}/{len(required_rows)}",
                   "pass" if not uncovered_artifacts else ("partial" if scenario_proofs["summary"]["scenario_gap_closed_rows"] else "gap"),
                   ["scenario-closure.yaml", "verification.plan.yaml", "evidence/scenarios/index.json", "evidence/scenarios/behaviors/"], uncovered_artifacts),
    ...
    return {
        ...
        "reference": {
            ...
            "scenario_gap_closure_source_commit": reference["scenario_gap_closure_reference"]["source_commit"],
            "scenario_gap_closure_unit": reference["scenario_gap_closure_reference"]["closure_unit"],
